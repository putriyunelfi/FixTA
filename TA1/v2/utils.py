...
def remove_multiple(file):
    links = [x.strip() for x in open('[1,2]links.txt', 'r+')
    ...
    with open('[1,2]links.txt', 'r+') as file:
...
def is_contain_keyword(link, keyword, session, cookie):
    ...
    wlcard = 'c.+.+n'
    pattern = re.compile(wlcard, keyword)
    result = soup.find_all(pattern, re.IGNORECASE)
