...
if __name__ == "__main__":
    ...
    for x in range(limit):
        ...
        remove_multiple('[10]links.txt')

    link_contain_keyword = []
    crawled_links = [link.strip() for link in open('[1,2]links.txt', 'r')]
    if len(crawled_links) != 0:  # proses crawling sebelumnya berhasil berarti jalanin
        ...
        with open(f'{k_word}[1,2].txt', 'w+') as file:
        #with open('cocain[10].txt', 'w+') as file:
